            ...
            <div class="card">
              ...
              <div class="card-body table-responsive p-0">
                <table id="datatable" class="table table-hover text-nowrap text-center">
                 ...
                 @foreach($meds as $med)
                  <tr>
                  <input type="hidden" class="delete_id" value="{{$med->med_id}}">
                  <td>{{$med->id}}</td>
                    <td>{{$med->Issue_date }}</td>
                    <td> {{$med->refernce}} </td>
                    <td>{{$med->passenger_name}}</td>
                    <td>{{$med->report_status}}</td>
                    <td>{{$med->document_number }} </td>
                    <td>{{$med->from_city }} </td>
                    <td>{{$med->med_info }} </td>
                    <td>{{$med->Dep_city}} </td>
                    <td> {{$med->dep_date}} </td>
                    <td> {{$med->arr_city}} </td>
                    <td>{{$med->sup_name}} </td>
                    <td>{{$med->provider_cost}} </td>
                    <td>{{$med->cur_name}}</td>
                    <td>{{$med->cost}}  </td>
                    <td> {{$med->passnger_currency}} </td>
                    <td>{{$med->remark}} </td>
                    <td>
