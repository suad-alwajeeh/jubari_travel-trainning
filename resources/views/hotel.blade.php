              ...
              <div class="card-body table-responsive p-0">
                <table id="datatable" class="table table-hover text-nowrap text-center">
                 ...
                 @foreach($hotels as $hotel)
                  <tr>
                  <input type="hidden" class="delete_id" value="{{$hotel->hotel_id}}">
                  <td>{{$hotel->id}}</td>
                    <td>{{$hotel->Issue_date }}</td>
                    <td> {{$hotel->refernce}} </td>
                    <td>{{$hotel->passenger_name}}</td>
                    <td>{{$hotel->hotel_status}}</td>
                    <td>{{$hotel->voucher_number }} </td>
                    <td>{{$hotel->Dep_city}} </td>
                    <td> {{$hotel->dep_date}} </td>
                    <td> {{$hotel->arr_city}} </td>
                    <td>{{$hotel->sup_name}} </td>
                    <td>{{$hotel->provider_cost}} </td>
                    <td>{{$hotel->cur_name}}</td>
                    <td>{{$hotel->cost}}  </td>
                    <td> {{$hotel->passnger_currency}} </td>
                    <td>{{$hotel->remark}} </td>
                    <td>
